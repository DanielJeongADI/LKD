		...
		const struct i2c_device_id *id)
{
	...
	me->chg_det = i2c_new_dummy(client->adapter, I2C_ADDR_CHARGER_DETECT);
	if (!me->chg_det) {
		rc = -ENOMEM;
		goto abort;
	}
	...
	if (unlikely(IS_ERR(me->regmap_chg_det))) {
...
static int max77840_i2c_remove(struct i2c_client *client)
{
