...
static int max77840_chgdet_get_input_current(struct max77840_chgdet_data
	*chgdet)
{
	u8 reg_data = 0;
...
static void max77840_chgdet_initialize(struct max77840_chgdet_data
	*chgdet)
{
	int rc;
	u8 val;
...
static int max77840_chgdet_probe(struct platform_device *pdev)
{
	...
	chgdet->psy_chgdet_d.num_properties	=
		ARRAY_SIZE(max77840_chgdet_props);
	...
	chgdet->chgtyp_irq = regmap_irq_get_virq(max77840->irqc_chgdet,
		CHGDET_IRQ_CHGTYPE_I);

	ret = request_threaded_irq(chgdet->chgtyp_irq, NULL,
			max77840_chgdet_chgin_isr,
			IRQF_TRIGGER_FALLING, "charger-detect-chgtyp", chgdet);
	if (ret) {
		pr_err("%s: Failed to Reqeust CHGTYP IRQ\n", __func__);
		goto err_chgtyp_irq;
	}
