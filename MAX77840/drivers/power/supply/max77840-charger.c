...
static bool max77840_charger_present_input(struct max77840_charger_data
	*charger)
{
	u8 chg_int_ok = 0;
	int rc;
...
static int max77840_charger_get_input_current(struct max77840_charger_data
	*charger)
{
	u8 reg_data = 0;
...
static int max77840_charger_get_charge_current(struct max77840_charger_data
	*charger)
{
...
static int max77840_charger_set_topoff_current(struct max77840_charger_data
	*charger,
...
static int max77840_charger_set_enable(struct max77840_charger_data
	*charger, int en)
{
...
static int max77840_charger_exit_dev(struct max77840_charger_data
	*charger)
{
...
static int max77840_charger_init_dev(struct max77840_charger_data
	*charger)
{
	int rc;
...
static void max77840_charger_initialize(struct max77840_charger_data
	*charger)
{
	struct max77840_charger_platform_data *pdata = charger->pdata;
	int rc;
	...
	*/
	...
	rc = max77840_charger_set_input_current(charger,
...
static int max77840_charger_update(struct max77840_charger_data *charger)
{
	...
	rc = max77840_bulk_read(charger->regmap,
	...
	charger->health =
		max77840_charger_status_map[chg_dtls].health;
	charger->status =
		max77840_charger_status_map[chg_dtls].status;
	charger->charge_type =
		max77840_charger_status_map[chg_dtls].charge_type;
...
static int max77840_charger_probe(struct platform_device *pdev)
{
	...
	charger->psy_chg_d.num_properties	=
		ARRAY_SIZE(max77840_charger_props);
	...
	charger->psy_chg =
		devm_power_supply_register(max77840->dev,
				&charger->psy_chg_d,
				&charger_cfg);
	...
	charger->byp_irq = regmap_irq_get_virq(max77840->irqc_chg,
		CHG_IRQ_BYP_I);
	charger->chgin_irq = regmap_irq_get_virq(max77840->irqc_chg,
		CHG_IRQ_CHGIN_I);
	charger->aicl_irq = regmap_irq_get_virq(max77840->irqc_chg,
		CHG_IRQ_AICL_I);
	charger->chg_irq = regmap_irq_get_virq(max77840->irqc_chg,
		CHG_IRQ_CHG_I);
