...
static uint16_t max77840_fg_get_version(struct max77840_chip *max77840_fg)
{
	uint16_t version;
	int rc;
...
static bool max77840_fg_check_status(struct max77840_chip *max77840_fg)
{
	...
	if (data & BIT_dSOCi) {
		...
		pr_info("max77840_fg_check_status soc changed, SOC=%d, VCELL=%d\n",
	...
	if (!ret) {
		data = data & 0x007F;
		fg_write_rt = max77840_fg_write(max77840_fg->regmap,
			REG_STATUS, data);
		if (fg_write_rt < 0)
			return ret;
	}
			...
			const char *buf, size_t size)
{
	...
	// register write
		...
		val = (int)simple_strtoul(args[2], NULL, 0);
...
static int max77840_fg_probe(struct platform_device *pdev)
{
	...
	chip->psy_batt_d.num_properties =
		ARRAY_SIZE(max77840_fg_battery_props);
	...
	chip->fg_irq = regmap_irq_get_virq(max77840->irqc_intsrc,
	  MAX77840_FG_INT);
