...
static struct regulator_ops max77840_safeout_ops = {
...
static int max77840_regulator_probe(struct platform_device *pdev)
{
	...
	max77840 = kzalloc(sizeof(struct max77840_data), GFP_KERNEL);
	if (!max77840)
		return -ENOMEM;
	...
	rdev = regulator_register(&max77840_safeout_desc,
		&config);
